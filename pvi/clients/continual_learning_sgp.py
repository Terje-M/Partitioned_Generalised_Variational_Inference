...
class ContinualLearningSGPClient(Client):
    ...
    def update_q(self, q): #, init_q=None)
        ...
        q_new, _ = self.gradient_based_update(q)
    ...
    def gradient_based_update(self, q):
        ...
        # Trying this so that q is not None else we set za to None
        # Previously without if else statement just the if part
        if q is not None:
        ...
        if za is not None:
            ...
            #Non standard implementation of ikaa
            ...
            sb_chol = torch.linalg.cholesky(sb)
        else:
            # Initialise Sb = Kbb.
            kbb = add_diagonal(self.model.kernel(zb, zb).detach(), JITTER)
            sb = kbb
    ...
    def gradient_based_update(self, q, qeps):
        ...
        for i in epoch_iter:
            ...
            for (x_batch, y_batch) in iter(loader):
                ...
                # Why sum(sum(...))
                #Prev: kleps = sum(sum(qeps.kl_divergence(peps).values())) / len(x)
                kleps = sum(qeps.kl_divergence(peps).values()) / len(x)
                

                ll = 0
                kl = 0 #? Why set kl = 0? Base case: kl = q.kl_divergence(q_cav).sum() / len(x)
