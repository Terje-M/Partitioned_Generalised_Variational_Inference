...
def psd_inverse(x=None, chol=None):
    ...
    """
    if chol is None:
        # torch.linalg.cholesky in favour of torch.cholesky
...
def safe_cholesky(x, min_eps=1e-8, max_eps=1e-2):
    ...
    while chol is None:
        try:
            #torch.cholesky(A) is deprecated in favour of torch.linalg.cholesky(A)
            #torch.cholesky(A, upper=True) replaced with torch.linalg.cholesky(A).mH
            chol = torch.linalg.cholesky(add_diagonal(x, eps))
        except RuntimeError:
            if eps >= max_eps:
                raise RuntimeError(
